...
while(<IN>) {
  ...
  if($c ==1) { 
    ...
    $insertLen = substr($remainingCigar, 0, index($remainingCigar, "D"));
    ...
    if(looks_like_number($insertLen)) 
    {
      ...
      $remainingCigar = substr($remainingCigar, index($remainingCigar, "D")+1);
	...
	if(exists($count{$_})) {
      ...
      print $lineArr[2] . "\t" . $insertStart . "\t" . $insertStart . "\t" . $insert . "\n";
    }
  }
}
