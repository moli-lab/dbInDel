...
while(<IN>)
{
	...
	if($lineArr[11] == 0 && $lineArr[12] == $readSize) {  #### IF THE WHOLE READ IS ALIGNED
		if($lineArr[5] < $readSize) {  #### SANITY CHECK FOR RIDICULOUSLY LONG DELETIONS
...
open(OUTBED, ">$ARGV[0].deletions.bed");
open(OUTSAM, ">$ARGV[0].sam");
open(OUTINSERTS, ">$ARGV[0].del.txt");
	...
	if($countHash{$bed} ==1 ) {
	  ...
	  print "---" . $blatStart . "---" . $blatChrom . "---" . $bowtieChrom . "---" . $bowtieStart . "\n";
	    ...
	    if($lineArr[1] == 0) {  # IF NO MISMATCHES
	      if($lineArr[6] == 1) {  # ONLY ONE INSERTION
		if($lineArr[7] < 20) { # INSERTION IS LT 20bp
